...
class _HomePageState extends State<HomePage> with SingleTickerProviderStateMixin {
  ...
  void _listenToSosAlerts() {
    ...
    sosRef.onChildAdded.listen((event) {
      final data = Map<String, dynamic>.from(event.snapshot.child('location').value as Map);
  ...
  Future<void> sendSosAlert() async {
    try {
      ...
      // Step 2: Handle location permission
      ...
      // Step 4: Load contact list
      final contactsSnapshot = await contactsRef.get();
      if (!contactsSnapshot.exists) {
        print('⚠️ No contacts found.');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('You have no contacts to send SOS'), backgroundColor: Colors.orange),
        );
        return;
      }

      final contacts = Map<String, dynamic>.from(contactsSnapshot.value as Map);
      print('👥 Contacts found: ${contacts.keys.join(', ')}');

      // Step 5: Send SOS to each contact
      ...
      print('❌ Error while sending SOS: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to send SOS'), backgroundColor: Colors.red),
      );
    }
  }
